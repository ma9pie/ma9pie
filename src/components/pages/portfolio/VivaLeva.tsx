...
const PROJECT_INFO = PORTFOLIO_PROJECTS[Project.VivaLeva];
...
const VivaLeva = () => {
  return (
    <>
      ...
      <Text.P>
        기존 담당자에게 프로젝트를 인수인계받아서 런칭 준비를 하였습니다. 하지만
        코드 내에서 리팩토링이나 비효율적인 부분들이 많이 보였었으나 한 번에
        바꾸기는 힘들 것 같아서 런칭 이후의 점진적인 리팩토링을 계획하였습니다.
        예시로는 기존의 useModal의 경우 단일 모달 처리만 되어있어 다중 모달
        처리가 가능한 hooks를 만들고 해당 hooks가 사용되는 컴포넌트들을 하나씩
        수정해 나갔습니다. 이외에도 core한 util이나 hooks를 새로 만들어 기존
        코드를 조금씩 수정해 나갔고, 총 2주간의 테스트와 리팩토링을 통해서 모두
        새로운 코드로 교체할 수 있었습니다. 기술 부채가 쌓이기 전, 런칭 초기에
        모두 잡겠다는 판단을 하였었고, 결론적으로는 추후 유지보수성이나 생산성의
        증대를 가져올 수 있었습니다.
      </Text.P>
      ...
      <Text.P>
        그리고 타사의 FE 프로젝트를 분석하면서, 아키텍처 구성방식, business
        logic 분리, 최적화 방법 등등이 어떤 식으로 구현되었는지 알 수 있었던
        점이 이후 개발에도 크게 도움을 주었던 것 같습니다.
      </Text.P>
    </>
  );
};
